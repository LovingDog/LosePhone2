...
public class AlarmInfoDao {
    ...
    public void addAlarmInfo(AlarmInfo alarmInfo){
        ...
        values.put(ConsUtils.ALARM_LOCATION_LON,alarmInfo.getLon());
        values.put(ConsUtils.ALARM_LOCATION_LAT,alarmInfo.getLat());
        values.put(ConsUtils.ALARM_LOCATION_NAME,alarmInfo.getLocation());
        values.put(ConsUtils.ALARM_DES,alarmInfo.getDesc());
        long time = db.insert(ConsUtils.ALARM_TABLE, null, values);
        Toast.makeText(mContext, "闹钟设置成功 = "+time, Toast.LENGTH_SHORT).show();
        if(db!=null){
            db.close();
            values.clear();
            values=null;
    ...
    public AlarmInfo findById(String alarmId){
        ...
        if(cursor!=null){
            ...
            if (cursor.moveToNext()){
                ...
                alarmInfo.setLocation(cursor.getString(cursor.getColumnIndex(ConsUtils.ALARM_LOCATION_NAME)));
                alarmInfo.setLon(cursor.getString(cursor.getColumnIndex(ConsUtils.ALARM_LOCATION_LON)));
                alarmInfo.setLat(cursor.getString(cursor.getColumnIndex(ConsUtils.ALARM_LOCATION_LAT)));
                alarmInfo.setDesc(cursor.getString(cursor.getColumnIndex(ConsUtils.ALARM_DES)));
            }
        }
        return alarmInfo;
    }
    ...
    public ArrayList<AlarmInfo> getAllAlarmLocationInfo(){
        ArrayList<AlarmInfo> list = (ArrayList<AlarmInfo>) getAllInfo();
    ...
    public List<AlarmInfo> getAllInfo(){
        ...
        if(cursor!=null){
            while(cursor.moveToNext()){
                ...
                alarmInfo.setLon(cursor.getString(cursor.getColumnIndex(ConsUtils.ALARM_LOCATION_LON)));
                alarmInfo.setLat(cursor.getString(cursor.getColumnIndex(ConsUtils.ALARM_LOCATION_LAT)));
                alarmInfo.setDesc(cursor.getString(cursor.getColumnIndexOrThrow(ConsUtils.ALARM_DES)));
                list.add(alarmInfo);
            }
            cursor.close();
        }
